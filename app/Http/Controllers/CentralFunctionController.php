    ...
    function createNewResult(){
        ...
        DB::beginTransaction();
            try
            {
               ...
               $activeDraws = DrawDetail::select('draw_masters.id as draw_master_id','draw_details.id as draw_detail_id','draw_details.draw_name')
                            ->join('draw_masters','draw_details.draw_master_id','draw_masters.id')
                            ->where('draw_details.draw_master_id',$lastDrawId)
                            ->get();
                foreach($activeDraws as $row){
                    $newData = DB::statement(
                        'CALL  insert_jodi_result('.$lastDrawId.','.$row->draw_detail_id.')'
                    );
                }


               // auto claim stopped as per demand 13.03.2021 for rajshree10(goarajshree.com)
...
//                   }
//               }
               // Auto claim removed as per sir's demand 08/03/2021
            ...
            //    if(!empty($barcodeWiseClaimDetails)){
