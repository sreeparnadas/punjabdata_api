...
class PersonController extends Controller
{
    ...
    public function validateCredential(request $request){
        ...
        if ((!empty($person) && empty($person->uuid)) || (!empty($person) && $person->person_category_id==1)){
            ...
            return response()->json($arrResponse,200);
        }elseif (!empty($person) && !empty($person->uuid)){
            $arrResponse = ['success'=>false,'isLoggedIn'=>false,'person_name'=>'','uuid'=>'','msg'=>'Account is already loggedIn'];
            return response()->json($arrResponse,200);
        }

        if (!empty($stockist) && empty($stockist->uuid)){
            ...
            return response()->json($arrResponse,200);
        }elseif (!empty($stockist) && !empty($stockist->uuid)){
            $arrResponse = ['success'=>false,'isLoggedIn'=>false,'person_name'=>'','uuid'=>'','msg'=>'Account is already loggedIn'];
            return response()->json($arrResponse,200);
        }
    }
    ...
    public function logOutUser(request $request){
        ...
        if($userCategoryId == 1 || $userCategoryId == 3){
            $person=Person::where(['id'=> $uid])->update(['is_loggedin'=>0,'uuid' => null]);
        }else if($userCategoryId == 4){
            $person=Stockist::where(['id'=> $uid])->update(['is_loggedin'=>0,'uuid' => null]);
        }
        return $person;
    }
